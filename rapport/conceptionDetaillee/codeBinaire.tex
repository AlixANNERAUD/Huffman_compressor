...
\constante{MAXIMUM\_LENGTH}{256} % TODO : à vérifier
...
\begin{algorithme}
    ...
    \procedure{remove\_bit}{\paramEntreeSortie{binary\_code : \binaryCode}, \paramEntree{index : \naturel}}
    {$0 \leqslant index < get\_length(binary\_code)$}
    {}
    {   \sialorssinon{get\_length(binary\_code) > 0}
        {   \pour{i}{index}{get\_length(binary\_code)-1}{}{
            \affecter{\champ{binary\_code}{bits}[i]}{\champ{binary\_code}{bits}[i+1]}
            }
            \affecter{\champ{binary\_code}{length}}{\champ{binary\_code}{length} - 1}}
        {}
    }
    ...
    \procedure{remove\_last\_bit}{\paramEntreeSortie{binary\_code : \binaryCode}}
    {$get\_length(binary\_code) > 0$}
    {}
    {   \sialorssinon{get\_length(binary\_code) > 0}% Est-ce que on garde cette ligne, et lignes 38, 49?
        {   \affecter{\champ{binary\_code}{length}}{\champ{binary\_code}{length} - 1}}
        {}
    }
